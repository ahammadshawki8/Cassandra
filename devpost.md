...
**The Verifier was too easy to fool — twice.**

- **Blanking a cell passed verification.** Pointing a formula at an empty cell satisfies both conditions: the value moved, and nothing else did. That made *destroying* a figure the cheapest way to pass. Now any patch that empties a cell which held a value is rejected outright.
- **A correct patch with a hallucinated prediction.** We deliberately fed the Verifier a *right* fix with a *wrong* predicted value. It rejected it: `predicted 999.0 but recalculation produced 100.0`. That test is now permanent.

**Per-repair figures did not match the file we shipped.**

- Each correction is verified independently against the original workbook — correct for *proving* one repair, wrong for *reporting* a result. Applied together, corrections interact: fixing the revenue range raises revenue, partly offsetting the opex sign correction.
- The interface said `-2,481,455`; the downloadable file computed `-1,704,250`. We now apply the full set at once and recalculate a final time. **The number on screen is the number in the file** — and a judge can verify that by opening it.

**Downstream is not the same as symptomatic.**

- When a total is wrong, everything below it is wrong too, so we suppressed findings inside a repaired cell's blast radius. That silently discarded **two genuine independent defects**, including the sign inversion responsible for the entire $7.95M swing.
- The fix: only a **purely semantic** complaint can be explained away by an upstream repair. If a structural detector read the cell's own formula and objected, no amount of fixing its inputs makes that cell correct.

**Detectors were reading an empty cache.**

- `openpyxl` exposes formulas *or* cached values, never both, and a workbook written by anything other than Excel has **no cached values at all**. Magnitude and polarity checks were silently declining to fire, and two of six planted defects went unfound. Detectors now read computed values from the calculation oracle.

**Cloud-specific traps worth passing on.**

- **Cloud Run reserves `/healthz`** at its frontend — it never reaches your container and returns Google's own 404 while every other route works perfectly. It reads exactly like a service that failed to start.
- **numpy scalars break `Content-Length`.** Values arriving from Firestore encode to a different byte length than the default encoder measures, so the response body overruns the header it already sent — mid-flight, on a route that looked fine locally.
- **`os.path.basename` only splits on the host's separator**, so a Windows path recorded by a local run came back whole from the Linux container. Our first fix did nothing to the exact row that prompted it.
...
- The audit is triggered by a file existing. Not by a chat prompt, not by a button. It runs for two minutes on its own, makes decisions, and produces a downloadable artifact. **The human enters only at accept-or-dismiss.**

**It has a ground-truth oracle, which almost no agent demo does.**

- Most agent projects ask a judge to *believe* that plausible-looking output is correct. Cassandra recalculates the workbook and shows the number move. And when it is wrong, **it says so and tries again** — on camera.
- The rubric explicitly asks how a system recovers when a worker agent returns a hallucination. Our answer is not a paragraph. It is a rejection message with the reason attached, followed by a successful second attempt.

**The engineering is deliberate, and the tradeoffs are defensible.**

- A clean separation between deterministic parsing and probabilistic judgement, with a stated rationale. Least privilege taken to its logical end. Idempotency on redelivery. Externalised, resumable state. Bounded retries that quarantine rather than guess.

**It is research-backed rather than research-adjacent.**

- Region clustering implements **CUSTODES (ICSE 2016)**. Verification-against-a-calculation-engine follows **arXiv 2508.11715**. The defect taxonomy mirrors **Panko and Halverson**. The prevalence figures are cited, not invented. We also reference **ExceLint** and Microsoft's **SpreadsheetLLM (arXiv 2407.09025)** for structural anomaly detection and grid encoding respectively.
- We are explicit about prior art — PerfectXL, Operis OAK, CUSTODES — because the contribution is not the detection. It is the **autonomous, continuous, self-verifying, regression-aware composition**, which does not exist as a product or a paper.

**And it does the unglamorous thing that separates a tool from a demo: it knows when to say nothing.**

- 123 correct formulas. Zero findings. A tool you can leave running.
